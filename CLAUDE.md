...
**Related provider-call fixes:** `MAX_OUTPUT_TOKENS` (3000, shared by all three providers) replaced the per-call `1200`; Gemini 2.5 Flash models get `thinkingConfig.thinkingBudget = 0` because thinking tokens are billed against `maxOutputTokens` and could consume the whole ceiling; and `extractGeminiText` / `extractOpenAIText` / `extractAnthropicText` join response parts with `""` instead of `"\n"` (the old join injected line breaks that were never in the output and broke JSON strings). Gemini thought parts and OpenAI `reasoning` / `refusal` items are now filtered out.
