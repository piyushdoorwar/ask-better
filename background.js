...
async function optimizePrompt(message) {
  ...
  if (!settings.enableAI || !isSiteEnabled(settings, site) || !settings.openaiApiKey) {
    return {
      ok: false,
  ...
  if (settings.provider !== "openai") {
    return {
      ...
      message: "Selected provider is not available yet."
    };
  }

  try {
    const optimizedPrompt = await callOpenAI({
...
function toPublicSettings(settings) {
  return {
    ...
    hasApiKey: !!(settings.openaiApiKey && settings.openaiApiKey.trim())
  };
}
